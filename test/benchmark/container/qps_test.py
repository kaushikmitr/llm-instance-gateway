...
async def send_request(
  ...
  request_latency = (prompt_len, output_len, (request_end_time - request_start_time)/(output_len))
...
async def main(args: argparse.Namespace):
    try:
        args.request_rates = json.loads(args.request_rates)
        args.num_prompts_values = json.loads(args.num_prompts_values)
        
        if len(args.request_rates) != len(args.num_prompts_values):
            raise ValueError("Length of request_rates must match length of num_prompts_values")
    except json.JSONDecodeError:
        raise ValueError("Both `--request-rates` and `--num-prompts-values` should be valid JSON strings.")

    # Prepare CSV file
    with open(CSV_FILENAME, mode='w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow([
            "Request Rate", "Num Prompts", "Model", 
            "Latency (mean)", "Latency (P90)", "Latency (min)", "Latency (max)", 
            "Throughput", "Total Prompts Attempted", "Total Prompts Succeeded", 
            "Total Tokens", "Errors"
        ])
        ...
        endpoint = "v1/completions" if args.backend == "vllm" else args.endpoint
        ...
        for request_rate, num_prompts in zip(args.request_rates, args.num_prompts_values):
            print(f"\nStarting benchmark with request_rate={request_rate}, num_prompts={num_prompts}")
            
            # Variables to store overall metrics
            overall_latency_list = []
            overall_errors = init_errors_map()
            ...
            # Run benchmark for all models with current configuration
            results = await asyncio.gather(
                *[benchmark(args, api_url, tokenizer, model) for model in models]
            )
            
            # Process results for each model separately
            for model, (latencies, errors) in zip(models, results):
                if not latencies:
                    print(f"No successful requests for model {model}")
                    continue
                    
                # Calculate metrics for this model
                total_output_tokens = np.sum([output_len for _, output_len, _ in
                                latencies])
                output_tokens_per_second = total_output_tokens / (time.time() - benchmark_start_time)
                avg_latency = np.mean([latency for _, _, latency in latencies])
                median_latency = np.median([latency for _, _, latency in latencies])
                min_latency = np.min([latency for _, _, latency in latencies])
                max_latency = np.max([latency for _, _, latency in latencies])
                p90_latency = np.percentile([latency for _, _, latency in latencies], 90)
                throughput = output_tokens_per_second
                total_model_tokens = sum(prompt_len + output_len for prompt_len, output_len, _ in latencies)
                
                # Update overall metrics
                overall_latency_list.extend(latency for _, _, latency in latencies)
                total_requests_attempted += num_prompts
                total_requests_succeeded += len(latencies)
                total_tokens += total_model_tokens
                for err, count in errors.items():
                    overall_errors[err] += count
                ...
                print(f"  Success rate: {len(latencies)}/{num_prompts} requests")
                print(f"  Average latency: {avg_latency:.4f} seconds")
                
            # Calculate and log overall metrics for this request rate
            if overall_latency_list:
                avg_overall_latency = np.mean(overall_latency_list)
                p90_overall_latency = np.percentile(overall_latency_list, 90)
                min_overall_latency = np.min(overall_latency_list)
                max_overall_latency = np.max(overall_latency_list)
                overall_throughput = total_tokens / (time.time() - benchmark_start_time)

                print(f"\n==== Overall Metrics for Request Rate {request_rate} ====")
                print(f"  Average Overall Latency: {avg_overall_latency:.4f} seconds")
                print(f"  90th Percentile Latency: {p90_overall_latency:.4f} seconds")
                print(f"  Overall Throughput: {overall_throughput:.2f} tokens/second")
                print(f"  Total Requests Attempted: {total_requests_attempted}")
                print(f"  Total Requests Succeeded: {total_requests_succeeded}")
                print(f"  Errors: {overall_errors}")

                # Write overall results for this request rate to CSV
                writer.writerow([
                    request_rate, 
                    num_prompts, 
                    "Overall",
                    f"{avg_overall_latency:.4f}", 
                    f"{p90_overall_latency:.4f}", 
                    f"{min_overall_latency:.4f}", 
                    f"{max_overall_latency:.4f}",
...
if __name__ == "__main__":
  ...
  parser.add_argument(
        ...
        type=str,
        default="[10,50,100]",
        help="JSON array of request rates (requests per second) to benchmark."
    )
  parser.add_argument(
        "--num-prompts-values",
        type=str,
        default="[500,1000,1500]",
        help="JSON array of number of prompts to benchmark (must match length of request-rates)."
    )
